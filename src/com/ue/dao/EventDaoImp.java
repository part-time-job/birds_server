...
public class EventDaoImp implements EventDao {
	...
	public boolean insertEvent(Event event) {
		...
		String sql = "insert into tb_event (" + "use_id, " + "eve_count, " + "eve_positivecount, " + "eve_question, "
				+ "eve_time, " + "eve_longitude, " + "eve_latitude, " + "eve_datatype, " + "eve_puretext, "
				+ "eve_audiourl, " + "eve_pictureurl, " + "eve_description, " + "eve_remark"
				+ ") values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
		LogUtils.sysoln(sql);
		int res = this.template.update(sql, event.getUse_id(), event.getEve_count(), event.getEve_positivecount(),
				event.getEve_question(), event.getEve_time(), event.getEve_longitude(), event.getEve_latitude(),
				event.getEve_datatype(), event.getEve_puretext(), event.getEve_audiourl(), event.getEve_pictureurl(),
				event.getEve_description(), event.getEve_remark());
		return res > 0;
	}
	...
	public List<Event> searchEventByPage(int pageNow, int pageSize, int dataType, boolean filterByUserId, int userId) {
		...
		if (filterByUserId == false) {
			if (ServerConfig.dataType_all.equalsIgnoreCase(ServerConfig.dataType)) {
				sql = "select * from tb_event where (eve_datatype=" + dataType + " or eve_datatype=" + (dataType + 100)
						+ " ) ORDER BY eve_id DESC limit ?," + pageSize;
			} else if (ServerConfig.dataType_cicada.equalsIgnoreCase(ServerConfig.dataType)) {
				dataType = dataType + 100;
				sql = "select * from tb_event where eve_datatype=" + dataType + " ORDER BY eve_id DESC limit ?,"
						+ pageSize;
			} else {
				sql = "select * from tb_event where eve_datatype=" + dataType + " ORDER BY eve_id DESC limit ?,"
						+ pageSize;
			...
			if (ServerConfig.dataType_all.equalsIgnoreCase(ServerConfig.dataType)) {
				sql = "select * from tb_event where use_id=" + userId + " and (eve_datatype=" + dataType
						+ " or eve_datatype=" + (dataType + 100) + ") ORDER BY eve_id DESC limit ?," + pageSize;
			} else if (ServerConfig.dataType_cicada.equalsIgnoreCase(ServerConfig.dataType)) {
				dataType = dataType + 100;
				sql = "select * from tb_event where use_id=" + userId + " and eve_datatype=" + dataType
						+ " ORDER BY eve_id DESC limit ?," + pageSize;
		...
		return (List<Event>) template.query(sql, new Integer[] { preCount },
	...
	public boolean updateEventByEventId(Event event) {
		// TODO Auto-generated method stub
		int res = this.template.update(UPDATE_BY_EVENT_ID, event.getUse_id(), event.getEve_count(),
				event.getEve_positivecount(), event.getEve_question(), event.getEve_time(), event.getEve_longitude(),
				event.getEve_latitude(), event.getEve_datatype(), event.getEve_puretext(), event.getEve_audiourl(),
				event.getEve_pictureurl(), event.getEve_description(), event.getEve_remark(), event.getEve_id());
		return res > 0;
	}

	@Override
	public List<Event> getAudioTopTen() {
		return (List<Event>) template.query(AUDIOTOPLIST, new Integer[] {},
				new BeanPropertyRowMapper<Event>(Event.class));
	}
	...
	public List<Event> searchEventByUserId(int userId) {
		// TODO Auto-generated method stub
		return (List<Event>) template.query(SEARCH_BY_USER_ID, new Integer[] { userId },
				new BeanPropertyRowMapper<Event>(Event.class));
	}
	...
	public List<Event> getAllEvent() {
		...
		if (ServerConfig.dataType_all.equalsIgnoreCase(ServerConfig.dataType)) {
			...
			sql = "select * from tb_event where (eve_datatype = " + Event.DATATYPE_PICTURE_cicada + " or eve_datatype = "
					+ Event.DATATYPE_AUDIO_cicada + " or eve_datatype = " + Event.DATATYPE_PURETEXT_cicada + ")";
		} else {
			sql = "select * from tb_event where (eve_datatype = " + Event.DATATYPE_PICTURE + " or eve_datatype = "
					+ Event.DATATYPE_AUDIO + " or eve_datatype = " + Event.DATATYPE_PURETEXT + ")";
		}
		List<Event> list = (List<Event>) template.query(sql, new BeanPropertyRowMapper<Event>(Event.class));
		return list;
	}

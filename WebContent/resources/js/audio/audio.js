...
function fillList(audio, i) {
	...
	li_html += "<div class='clear text-ellipsis'>";
	li_html += "<img class=\"img-circle\" height='40px' width='40px' id='myPlayer2"+audio.eve_id+"' src='../resources/images/play.jpg' onclick='palyOrPause2("+audio.eve_id+");' /><audio onended='changeIcon("+audio.eve_id+");' id='ap2"+audio.eve_id+"' preload=\"none\" src='../user/audio/" + audio.eve_audiourl
			+ "'>";
	li_html += "Your browser does not support the audio element.</audio>";
	
//	li_html += "<audio preload=\"none\" style=\"width: 50%\" src='" + audio.eve_audiourl
//			+ "' >";
//	li_html += "Your browser does not support the audio element.</audio>";
...
//	li_html += "<a href='../audio/getDetail?eve_id="+audio.eve_id+"'>";
	li_html += "<span><a href='../index/audioDetail?eve_id="+audio.eve_id+"' target='mainframe'>查看详情</a></span>";
//	li_html += "</a>";
	li_html += "</div>";
	li_html += "</li>";
	return li_html;
...
function fillUl(audio) {
	...
	li_html += "<div class='pull-right m-l'>";
	li_html += "<a href='#' onclick='good(" + audio.eve_id + ");'>";
	li_html += "<i class='glyphicon glyphicon-thumbs-up'>" + " <span id='"
			+ audio.eve_id + "' >" + audio.eve_positivecount + "</span>";
	li_html += "</i>";
	li_html += "</a>";
	li_html += "</div>";
	
	li_html += "<div class='clear text-ellipsis'>";
	li_html += "<img class=\"img-circle\" height='40px' width='40px' id='myPlayer"+audio.eve_id+"' src='../resources/images/play.jpg' onclick='palyOrPause("+audio.eve_id+");' /><audio onended='changeIcon("+audio.eve_id+");' id='ap"+audio.eve_id+"' preload=\"none\" src='../user/audio/" + audio.eve_audiourl
			+ "'>";
	li_html += "Your browser does not support the audio element.</audio>";
	li_html += "<span ><font color='white' size='3'> &nbsp;&nbsp" + audio.eve_description + "&nbsp" + audio.eve_time +" </font></span>";
	li_html += "</div>";
	...
	li_html += "<img class=\"img-circle\" height='40px' width='40px' id='myPlayer"+audio.eve_id+"' src='../resources/images/play.jpg' onclick='palyOrPause("+audio.eve_audiourl + ","+ audio.eve_id + ");' />";
...
//	li_html += "<audio preload=\"none\" style=\"width: 30%\" src='" + audio.eve_audiourl
//			+ "' id='ap' controls='controls'>";
//	li_html += "Your browser does not support the audio element.</audio>";

	li_html += "<span>" + audio.eve_description + "&nbsp" + audio.eve_time +"</span>";
...
var playingAudio;
var isPlaying = false;
function palyOrPause2(id){
	var playControl = document.getElementById("ap2" + id);
	var playIcon = $("#myPlayer2" + id);
	playIcon.attr("src", "");
...
/*var isPlaying = false;
function palyOrPause(url, id){
	alert(url + id);
	var playControl = document.getElementById("ap");
	var playIcon = $("#myPlayer" + id);
	playControl.attr("src", url);
	playIcon.attr("src", "");
	if(isPlaying){
		isPlaying = false;
		playControl.pause();
